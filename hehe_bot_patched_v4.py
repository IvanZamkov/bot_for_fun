...
UI = {
    "ru": {
        ...
        "inline_shake_desc_with_target": "Ваша цель: {target}, действуй)",
        ...
        "inline_guard_text": "🛡Телохранитель\n\nНажми «{btn}», чтобы защитить себя за 400ꙓ.",
    ...
    "en": {
        ...
        "inline_shake_desc_with_target": "Your goal: {target}, act now)",
        ...
        "inline_guard_text": "🛡Bodyguard\n\nPress “{btn}” to protect yourself for 400ꙓ.",
...
def get_setup_msg(uid: int):
    ...
    return cur.fetchone()  # (msg_id, stage)
...
def register_user_group(group_key, inline_message_id, user):
    try:
        cur.execute(
            "INSERT INTO registrations (id, group_key, inline_message_id, user_id, username, first_name, last_name, ts) "
            "VALUES (?,?,?,?,?,?,?,?)",
            (str(uuid.uuid4()), group_key, inline_message_id, user.id,
        ...
        conn.commit()
        logger.info("Registered user %s for group %s (inline_id=%s)", user.id, group_key, inline_message_id)
        return True
...
def ensure_user_stats(uid, username=None, first_name=None, last_name=None):
    ...
    if not cur.fetchone():
        ...
        cur.execute("UPDATE stats SET username=?, first_name=?, last_name=?, last_update=? WHERE user_id=?",
...
def update_stats_on_result(uid, username, first_name, last_name, success_delta=0, total_delta=0):
    ...
    cur.execute(
        "UPDATE stats SET success_count=?, total_count=?, username=?, first_name=?, last_name=?, last_update=? WHERE user_id=?",
...
def ensure_user_stats_chat(group_key: str, uid: int, username=None, first_name=None, last_name=None):
    ...
    if not cur.fetchone():
        ...
        cur.execute(
            "UPDATE stats_chat SET username=?, first_name=?, last_name=?, last_update=? WHERE group_key=? AND user_id=?",
...
def update_stats_chat_on_result(group_key: str, uid: int, username=None, first_name=None, last_name=None, success_delta=0, total_delta=0):
    ...
    t_ = max(0, t_ + total_delta)
    cur.execute(
        "UPDATE stats_chat SET success_count=?, total_count=?, username=?, first_name=?, last_name=?, last_update=? "
...
def get_chat_state(group_key: str, uid: int):
    ...
    cur.execute("SELECT last_target_key, last_target_block_rem, last_rand1, last_rand2 FROM chat_state WHERE group_key=? AND user_id=?",
                (group_key, uid))
    return cur.fetchone()  # (last_target_key, rem, rand1, rand2)
...
def on_confirm(call: CallbackQuery):
    ...
    if kind == "shake":
        ...
        curr_count, *_ = get_attempts(user.id)
        if curr_count >= 3:
    ...
    if kind == "guard":
        ...
        have = get_points_chat(group_key, user.id)
        if have < 400:
            text = ui(user.id, "not_enough_points").format(need=400, have=have)
            try:
                if inline_id:
                    bot.edit_message_text(text, inline_message_id=inline_id, parse_mode="HTML")
                elif getattr(call, "message", None):
        ...
        spend_points_chat(group_key, user.id, 400)
...
def cmd_admin_delete_user(message):
    ...
    bot.reply_to(message, f"Удалены данные пользователя {uid} (stats/stats_chat/registrations/attempts/settings).")
