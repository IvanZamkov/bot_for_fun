...
BOT_TOKEN = os.environ.get("BOT_TOKEN") or "8393536900:AAH3D0Hn4cYQ9QEpPAle2eCr0kbjg_4IXF0"
...
UI = {
    "ru": {
        ...
        "inline_contest_desc": "Ставки, шлюхи, три топора - проверь свою удачу",
    ...
    "en": {
        ...
        "inline_contest_desc": "Bets, whores, three axes - test your luck",
...
def format_name_html(username: str | None, first_name: str | None, last_name: str | None, uid: int) -> str:
    ...
    # no first/last name -> fallback to username (not id)
    if username:
...
def register_user_group(group_key, inline_message_id, user):
    ...
    try:
        ...
        # Also refresh stats tables (but NEVER overwrite good data with None)
        ensure_user_stats(
            user.id,
            getattr(user, "username", None),
            getattr(user, "first_name", None),
...
def _apply_shop_effect(group_key: str, uid: int, item_key: str):
    ...
    if item_key == "coffee":
        ...
        # 2 = next is forced win; then forced loss
    ...
    try:
        ...
        if kind == "contest":
            ...
            if action == "roulette":
                ...
                # show result + play again button if attempts left
...
def on_confirm(call: CallbackQuery):
    ...
    if kind == "shake":
        ...
        if group_key:
            ...
            # coffee bonus applies only within current cooldown window
        ...
        # wine: forced win, then forced loss
...
def _maintenance_tick():
    ...
    now = int(time.time())
    try:
        # Fill missing stats_chat names from registrations (same group/user)
        cur.execute(
            """
            UPDATE stats_chat
            SET
        ...
        # Fill missing global stats names from latest stats_chat row
        cur.execute(
            """
            UPDATE stats
            SET
